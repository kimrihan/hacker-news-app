        ...
        <div class="p-6 ${
          ...
          <div class="flex mt-3">
            <div class="grid grid-cols-3 text-sm text-gray-500">
              <div class="fas fa-user mr-1">${user}</div>
              <div class="fas fa-heart mr-1">${points}</div>
              <div class="fas fa-clock mr-1">${time_ago}</div>
            </div>
          </div>
        </div>
      `);
		...
		this.setTemplateData(
			'next_page',
			String(
				store.currentPage < Math.ceil(this.feeds.length / 10)
					? store.currentPage + 1
					: store.currentPage
			)
		);
...
class NewsDetailView extends View {
	constructor(constainerId: string) {
		let template = /*html*/ `
